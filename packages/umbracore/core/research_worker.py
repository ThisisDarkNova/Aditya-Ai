...
"""
Background research worker system for Vespera.
...
from core.web_services import web_search, httpx_client
import re
import urllib.parse

logger = logging.getLogger("vespera-research-worker")
...
def _run_research(job: ResearchJob):
    ...
    job.status = "running"
    job.add_step("Initializing Vespera Autonomous Research Agent...")
...
def _synthesize_summary(query: str, sources: List[Dict[str, str]]) -> str:
    ...
    md.append(f"\n*Generated by Vespera Autonomous Research Engine on {time.strftime('%Y-%m-%d %H:%M:%S')}*\n")
