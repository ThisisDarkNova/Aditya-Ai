...
"""
Startup Manager for Vespera.
Uses the Windows Registry (HKCU Run) to register Vespera for auto-start.
...
logger = logging.getLogger("vespera-startup")
...
def get_startup_command() -> str:
    """
    Get the command-line string to launch Vespera, prioritizing VESPERA.exe if packaged.
    """
    if getattr(sys, "frozen", False):
        # In production (PyInstaller frozen), VesperaCore.exe is inside 'resources/' of the NSIS install
        base_dir = Path(sys.executable).resolve().parent
        if base_dir.name == "resources":
            exe_path = base_dir.parent / "VESPERA.exe"
            if exe_path.exists():
...
def handle_delayed_startup() -> None:
    ...
    if "--startup" in sys.argv:
        ...
        logger.info(f"Vespera launched via Windows startup. Delaying boot by {delay} seconds...")
        time.sleep(delay)
