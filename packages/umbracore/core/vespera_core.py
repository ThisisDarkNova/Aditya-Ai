# core/vespera_core.py
"""
╔══════════════════════════════════════════════════════════════╗
║              VESPERA Core — Command Router                    ║
║  Central brain that routes every spoken/typed command to     ║
║  the correct skill, module, or OS function.                  ║
║                                                              ║
║  Every OS-level action uses VesperaSilentLauncher:            ║
...
logger = logging.getLogger("vespera-core")

# ── Allowed raw system commands (allowlist for security) ──────────────────────
# Vespera will NOT execute raw shell strings unless they start with one of these.
_SAFE_CMD_PREFIXES = (
    "taskkill",
    "explorer",
    "ipconfig",
...
def _process_single_command(command: str) -> str:
    ...
    All OS-level launches go through VesperaSilentLauncher.
    Returns a human-readable result string for Vespera to speak.
    """
    import time
    ...
    # ── 6. MOUSE ─────────────────────────────────────────────────────────────
        ...
        from core.app_opener import mouse_control
    ...
    # ── 12. SYSTEM POWER ──────────────────────────────────────────────────────
        ...
        run_tool_silently("shutdown.exe", ["/s", "/t", "5", "/c", "Vespera initiated shutdown."])
        return "PC will shut down in 5 seconds."

    elif command_lower in ("restart", "restart pc"):
        run_tool_silently("shutdown.exe", ["/r", "/t", "5", "/c", "Vespera initiated restart."])
    ...
    # ── 13. VESPERA STANDBY ────────────────────────────────────────────────────
    elif command_lower in ("standby", "vespera sleep", "sleep vespera", "go to sleep", "deactivate wake"):
        try:
            from core.wake_system import wake_system
            wake_system.reset_trigger()
            return "Vespera entering standby mode."
        except Exception:
