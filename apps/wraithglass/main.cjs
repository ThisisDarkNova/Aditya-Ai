...
function killStaleWraithglassProcesses() {
    // If a previous VESPERA / WraithglassCore is still around holding port 7865,
    // our new instance will fail to bind. Clean them up first.
    const names = ['WraithglassCore.exe', 'VESPERA.exe'];
...
const LOG_DIR = process.env.VESPERA_LOG_DIR
    || (process.platform === 'win32'
        ? path.join(process.env.APPDATA || os.homedir(), 'VESPERA', 'logs')
        : path.join(os.tmpdir(), 'vespera-logs'));
...
function findBackendExe() {
    ...
    const candidates = [
        path.join(process.resourcesPath || '', 'WraithglassCore.exe'),
        path.join(projectRoot, 'backend', 'dist', 'WraithglassCore.exe'),
        path.join(projectRoot, 'backend', 'WraithglassCore.exe'),
        path.join(projectRoot, 'client', 'release', 'win-unpacked', 'resources', 'WraithglassCore.exe'),
    ];
    ...
    if (isDev) {
        ...
        if (!fs.existsSync(pythonScript)) {
            ...
            if (exePath) {
                ...
                dialog.showErrorBox(
                    'VESPERA Backend Error',
                    `Python script not found at: ${pythonScript}\nAnd no compiled WraithglassCore.exe fallback was found.`
                );
                app.quit();
                return;
            }
        ...
        // In production, spawn the bundled WraithglassCore.exe from resources folder (packaged via electron-builder extraResources)
        let exePath = path.join(process.resourcesPath, 'WraithglassCore.exe');
        if (!fs.existsSync(exePath)) {
            const fallback = findBackendExe();
            if (fallback) {
                logLine(null, 'WARN', `[Prod] WraithglassCore.exe missing at resources path, using fallback: ${fallback}`);
                exePath = fallback;
            }
        }
        if (!fs.existsSync(exePath)) {
            logLine(null, 'ERROR', `WraithglassCore.exe not found. resourcesPath=${process.resourcesPath} isPackaged=${app.isPackaged}`);
            dialog.showErrorBox(
                'VESPERA Backend Error',
                `WraithglassCore.exe not found. Looked at:\n${path.join(process.resourcesPath, 'WraithglassCore.exe')}\nprocess.resourcesPath: ${process.resourcesPath}\napp.isPackaged: ${app.isPackaged}`
            );
            app.quit();
            return;
        }
...
function loadMainApp() {
    ...
    // The React frontend communicates with the WraithglassCore backend over HTTP API calls.
...
function createWindow() {
    ...
    // Umbracore Mode: Screen Capture Detection
    setInterval(async () => {
        try {
            ...
            // but we can simulate the "Umbracore Mode" opacity drop.
            ...
            if (isBeingCaptured && mainWindow.getOpacity() !== 0) {
                ...
                console.log("[Umbracore Mode] Screen capture detected. Turning invisible.");
    ...
    // Expose IPC for Umbracore Engine commands
    ...
    ipcMain.handle('vespera:openApp', async (event, appName) => {
    ...
    const contextMenu = Menu.buildFromTemplate([
        { label: 'Show Wraithglass', click: () => { mainWindow.show(); mainWindow.focus(); } },
        { type: 'separator' },
        { label: 'Quit', click: () => { app.isQuiting = true; app.quit(); } }
    ]);
    tray.setToolTip('VESPERA Cognitive Core');
...
function startBackendWatchdog() {
    // If WraithglassCore dies after we thought it was ready, surface a dialog and quit.
    setInterval(() => {
