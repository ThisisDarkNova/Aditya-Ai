...
plays activation sounds, and restores Vespera user interfaces.
"""
...
logger = logging.getLogger("vespera-wake-orchestrator")
...
class WakeSystemOrchestrator:
    ...
    def request_microphone_permission(self) -> bool:
        ...
        with self._lock:
            ...
            print("\n" + "!"*50)
            print("🎙️ VESPERA Microphone Access Authorization Request")
            print("!"*50)
            print("Vespera requires microphone access to support:")
            ...
            print("No audio data is sent to the cloud until you activate Vespera.")
            print("!"*50)
            ...
            response = input("Grant microphone permission to Vespera? (y/n): ").strip().lower()
    ...
    def wake(self) -> None:
        ...
        with self._lock:
            ...
            if self.triggered:
                return
            self.triggered = True
            logger.info("Vespera has been awakened! Executing wake sequence...")

            # 1. Play activation sound asynchronously
            self._play_activation_sound()

            # 2. Restore/focus UI Windows
            self._restore_vespera_windows()
            ...
            # 4. Inject alert to prompt Vespera to speak immediately
            try:
                ...
                reminder_system.alerts.append(
                    ...
                    "Instantly say a brief, warm greeting (e.g. 'I am listening!', 'Vespera online.') "
    ...
    def _restore_vespera_windows(self) -> None:
        """
        Search for and bring Vespera windows to the foreground.
        """
        try:
            windows = gw.getAllWindows()
            found = False
            for w in windows:
                if w.title and "VESPERA" in w.title.upper():
                    try:
                        w.restore()
                        w.activate()
                        found = True
                        logger.info(f"Restored Vespera interface window: '{w.title}'")
                    except Exception as e:
                        logger.debug(f"Could not restore window '{w.title}': {e}")
            if not found:
                logger.debug("No Vespera windows found to restore.")
        except Exception as e:
