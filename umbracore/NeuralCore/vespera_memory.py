...
from NeuralCore.SilentSealCrypto import engine

_DATA_DIR = get_data_dir()
_DEFAULT_MEMORY_PATH = str(_DATA_DIR / "memory.bin") # Changed to .bin for encrypted data


import threading

class VesperaMemoryEngine:
    ...
    def load(self):
        if not os.path.exists(self.path):
            # If migrating from old memory.json, attempt to load it and encrypt
            old_json = self.path.replace(".bin", ".json")
            if os.path.exists(old_json):
                try:
                    with open(old_json, "r", encoding="utf-8") as f:
                        data = json.load(f)
                    # Don't delete the old JSON right away, but the new encrypted one will be saved shortly
                    return data if isinstance(data, dict) else self._default_empty()
                except Exception:
                    pass
        ...
        try:
            with open(self.path, "rb") as f:
                ...
                data = engine.decrypt_data(encrypted_bytes)
    ...
    def save(self):
        with self._lock:
            ...
            try:
                encrypted_bytes = engine.encrypt_data(self.data)
    ...
    def update_profile(self, key, value):
        ...
        self.save()
        try:
            print(f"[💾 VesperaMemoryEngine] Permanently saved to disk: {key} = {value}")
        except Exception:
            print(f"[VesperaMemoryEngine] Permanently saved to disk: {key} = {value}")
...
memory_system = VesperaMemoryEngine()
