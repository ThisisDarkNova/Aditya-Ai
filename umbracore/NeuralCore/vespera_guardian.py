...
"""
Disk space and system memory leak prevention guard for Vespera.
...
logger = logging.getLogger("vespera-resource-guard")
logger.setLevel(logging.INFO)

class VesperaGuardian:
    ...
    def start(self):
        self._running = True
        t = threading.Thread(target=self._run_guard, name="VesperaGuardianThread", daemon=True)
        t.start()
        ...
        while self._running:
            ...
            try:
                ...
                if mem_mb > 500: # Python engine shouldn't consume > 500MB RAM typically
                    logger.warning(f"Vespera memory consumption high ({mem_mb:.1f}MB). Triggering garbage collection.")
                    import gc
                    gc.collect()
...
resource_guard = VesperaGuardian()
