...
from NeuralCore.skills_manager import (
    test_python_code,
    save_tested_skill,
    execute_skill,
...
from NeuralCore.aditya_runtime import runtime
from NeuralCore.model_config import get_fallback_chain
from NeuralCore.aditya_voice import realtime, ResponseMode, NetworkMode
...
from NeuralCore.paths import get_logs_dir

log_file = get_logs_dir() / "aditya.log"
logging.basicConfig(
...
logger = logging.getLogger("aditya-live")
...
def _build_config(resume_handle: Optional[str]) -> types.LiveConnectConfig:
    ...
    # Inject AdityaMemoryEngine Context
        ...
        system_instruction=types.Content(
            parts=[
                types.Part.from_text(
                    text=ADITYA_PERSONA + context_str + skills_context + sys_status_str
                )
            ]
        ),
        tools=[
            {
                "function_declarations": [
                    ...
                    },
                    {
                        ...
                        "description": "Visual test: Moves the mouse to the corners of the screen. Use this to verify if Hinata's scaling and cursor positioning are accurate.",
                        "parameters": {
                            "type": "OBJECT",
                            "properties": {},
                        },
...
class SpeakerPipeline:
    ...
    def try_put(self, data: bytes) -> None:
        ...
        try:
            ...
            # Drop oldest to bound latency (Aditya-like: prefer fresh audio)
            try:
        ...
        if typed_msg:
            try:
                ...
                if mode == ResponseMode.CACHE:
                    ...
                    from ui_server import _lock as _ui_lock, _pending_inputs as _ui_pending
...
def _bg_run(func, *args, name_label="Task"):
    async def _task():
        ...
        from ui_server import _lock, _pending_inputs, add_chat_message
    ...
    while not stop.is_set():
        ...
        if hasattr(msg, "tool_call") and msg.tool_call:
            ...
            for fc in msg.tool_call.function_calls:
                ...
                set_status("tool")
                print(
                    f"\n[* Aditya running tool: {name}({args_dict})] ... ",
                    end="",
                    flush=True,
                )
                ...
                try:
                    if name == "process_command":
                        ...
                        from NeuralCore.aditya_core import process_command
                        ...
                        from NeuralCore.aditya_knowledge import learning_system
                            ...
                            try:
                                ...
                                def call_models(prompt_text):
                                    ...
                                    for _model_name in _content_chain:
                                        try:
                                            print(
                                                f"[Aditya: Attempting {_model_name}...]",
                                                flush=True,
                                            )
                                ...
                                print(
                                    f"\n[* Aditya: Analyzing prompt and file {filename} ...]"
                                )
                                existing_content = ""
                                ...
                                if is_code:
                                    ...
                                    print(
                                        "[* Aditya: Converting to PRO Expert Prompt (Phase 0)...]"
                                    )
                                    phase0_prompt = (
                                    ...
                                    print(
                                        "[* Aditya: Generating initial code/text using Expert Prompt (Phase 1)...]"
                                    )
                                    initial_content = call_models(expert_prompt)

                                    print(
                                        "[* Aditya: Reviewing and perfecting generated content (Phase 2)...]"
                                    )
                                    refine_prompt = (
                                    ...
                                    print(
                                        "[* Aditya: Generating standard text content directly...]"
                                    )
                        ...
                        async def _bg_image():
                            try:
                                from NeuralCore.image_generator import generate_image_with_fallback
                        ...
                        try:
                            from NeuralCore.aditya_society import autonomous_agent
                        ...
                        from V12Cylinders.desktop_ops import list_windows
                        result = await asyncio.to_thread(list_windows)
                    elif name == "manage_windows":
                        from V12Cylinders.desktop_ops import organize_windows
        ...
        if sc.interrupted:
            print("\n[You interrupted Aditya]", flush=True)
            speaker.flush()
...
async def _send_alerts_forever(
    ...
    """Polls reminder_system.alerts and injects them as system prompts to force Aditya to speak."""
    while not stop.is_set():
        try:
            from ui_server import check_and_clear_interrupt, set_status
                        ...
                        try:
                            ...
                            if len(clean_new) > 5 and clean_new != clean_old:
                                if (
                                    ...
                                    # otherwise Aditya never actually receives the text invisibly, only the UI does!
                                    from ui_server import (
                                        _lock,
                                        _pending_inputs,
                                        add_chat_message,
...
async def _session_rotation_manager(resume_state: dict, stop: asyncio.Event) -> None:
    ...
    try:
        ...
        # Wait until Aditya finishes speaking
...
async def main() -> None:
    ...
    while True:
        try:
            ...
            try:
                from NeuralCore.audio_recovery import audio_monitor
    ...
    from NeuralCore.keyboard_listener import global_hotkey_system
    global_hotkey_system.start()

    print("Connecting to Gemini Live (Gemini 3.1). Start talking to Aditya...")
    print("Ctrl+C to exit.\n")
    set_status("connecting")

    # Start Autonomous Agent background brain
    try:
        from NeuralCore.aditya_society import AdityaSociety
        from ui_server import _lock, _pending_inputs
...
if __name__ == "__main__":
    ...
    from V12Cylinders.startup_manager import handle_delayed_startup, sync_startup_with_registry
    ...
    time.sleep(0.3)
    print("Aditya is awake.\n")
    time.sleep(0.2)

    # Initialize process supervisor watchdog & resource guard daemons
    from NeuralCore.aditya_kernel_supervisor import supervisor
    from NeuralCore.aditya_guardian import resource_guard
    from NeuralCore.signal_handler import shutdown_coordinator
    ...
    from NeuralCore.db_doctor import check_and_repair_db
    from NeuralCore.aditya_memory import memory_system
    ...
    # Start the Threaded Metric Monitor (Legacy Aditya v3.0 Feature)
    from NeuralCore.system_monitor import sys_monitor
    from NeuralCore.proactive_helper import proactive_helper
    ...
    def compute_reserve_monitor():
        ...
        while True:
            try:
                ...
                if load > 85:
                    print(f"[COMPUTE RESERVE] Critical CPU load ({load}%). Throttling PhantomEngine background tasks to guarantee 100% FPS.")
    ...
    def silent_seal_loop():
        ...
        engine_thread_pool = ThreadPoolExecutor(max_workers=4, thread_name_prefix="PhantomEngine_Worker")
        ...
        while True:
            try:
                ...
                print(f"\\n[SILENT SEAL] PhantomEngine crashed: {e}")
                print("[SILENT SEAL] Gracefully recalibrating and restarting in 2 seconds...", flush=True)
                time.sleep(2)
                
    assistant_thread = threading.Thread(
        target=silent_seal_loop, name="PhantomEngine_Main", daemon=True
    )
