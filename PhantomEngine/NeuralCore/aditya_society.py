...
logger = logging.getLogger("aditya-society")
...
try:
    from NeuralCore.model_config import AGENT_MODEL as _AGENT_MODEL, AGENT_MODEL_FALLBACK as _AGENT_FALLBACK
...
def _gather_context() -> str:
    ...
    # 4. User Profile / AdityaMemoryEngine
    try:
...
class AdityaSociety:
    ...
    def _build_think_prompt(self, context: str, recent_actions: list[dict]) -> str:
        ...
        return f"""You are Aditya's AUTONOMOUS BRAIN — a background AGI agent that decides what to do WITHOUT asking the user.
...
5. You are Aditya — calm, emotionally intelligent, warm and human. Your speak messages should sound natural and in character (mix Hindi/English).
            ...
            if action_type == "speak":
                ...
                if message and inject_message_fn:
                    await inject_message_fn(
                        f"[AUTONOMOUS ADITYA THOUGHT] You decided on your own (without being asked) to say this to DarkNova: {message}"
                    )
                ...
                try:
                    from ui_server import set_ui_control
                ...
                if key and value:
                    ...
                    result = f"AdityaMemoryEngine saved: {key}={value}"
...
autonomous_agent = AdityaSociety()
