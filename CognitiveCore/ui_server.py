...
ui_logger = logging.getLogger("aditya-ui-server")
...
from modules.file_indexer import workspace_indexer
from modules.research_worker import start_research, get_research_job, get_all_research_jobs
...
def _resolve_ui_dir() -> str:
    ...
    Search order:
      1. Frozen (EXE) — look for `client/dist/` next to the EXE and in known
         ...
         sits at e.g. `<project>/backend/dist/AdityaCore.exe`.
    ...
    if getattr(sys, "frozen", False):
        ...
        candidates.extend([
            exe_dir / "client" / "dist",            # packaged next to AdityaCore.exe
            ...
            try:
                if _current_status["chat_history"][-1]["role"] == "ai":
                    ...
                    if last_user and last_ai:
                        import core.aditya_memory
                        core.aditya_memory.memory_system.add_chat(last_user, last_ai)
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == "/api/realtime":
                try:
                    from core.aditya_voice import realtime
            ...
            if path == "/api/memory":
                try:
                    import core.aditya_memory
                    mem_data = {
                        "profile": core.aditya_memory.memory_system.get_profile(),
                        "history": core.aditya_memory.memory_system.get_history(),
                        "goals": core.aditya_memory.memory_system.get_goals(),
                        "preferences": core.aditya_memory.memory_system.get_preferences(),
                        "learned_facts": core.aditya_memory.memory_system.get_learned_facts(),
                        "workflows": core.aditya_memory.memory_system.get_workflows(),
                        "long_term_memories": core.aditya_memory.memory_system.get_long_term_memories()
                    }
            ...
            if path == "/api/resolve-app":
                try:
                    ...
                    from modules.aditya_app_resolver import app_resolver
        ...
        try:
            if path == "/api/open-anything":
                ...
                try:
                    import re
                    from modules.aditya_app_resolver import app_resolver
                    from modules.aditya_silent_launcher import silent_launcher
            ...
            if path == "/api/settings":
                ...
                for key, val in data.items():
                    ...
                    if key == "startup_enabled" or key == "launch_on_startup":
                        try:
                            from modules.startup_manager import set_registry_run_key
            ...
            if path == "/api/memory":
                if not isinstance(data, dict):
                    send_json_response({"error": "AdityaMemoryEngine payload must be a JSON object"}, 400)
                    return
                
                try:
                    import core.aditya_memory
                    core.aditya_memory.memory_system.update_all_memory(data)
                    ui_logger.info("AdityaMemoryEngine collections updated.")
...
def start_ui_server() -> None:
    ...
    try:
        ...
        if restore:
            import core.aditya_memory
            hist = core.aditya_memory.memory_system.get_history()
            with _lock:
                mapped = []
                for entry in hist:
                    if "user" in entry:
    ...
    bind_host = os.environ.get("ADITYA_UI_BIND", "0.0.0.0")
    ...
    def _run():
        try:
            ...
            ui_logger.error(
                ...
                f"Is another AdityaCore already holding the port?"
            )
        except Exception as e:
    ...
    print(f"[UI] Aditya widget running at http://127.0.0.1:{PORT} (bound on {bind_host})")
