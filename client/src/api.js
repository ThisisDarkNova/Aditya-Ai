 ...
 * Fetch memory data
 */
export async function fetchAdityaMemoryEngine() {
  try {
    const res = await stableFetch(`${API_BASE}/api/memory`);
    if (!res.ok) throw new Error(`AdityaMemoryEngine ${res.status}`);
    return await res.json();
  } catch (err) {
 ...
 * Update memory items
 */
export async function updateAdityaMemoryEngine(data) {
  try {
    const res = await stableFetch(`${API_BASE}/api/memory`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) throw new Error(`AdityaMemoryEngine update failed: ${res.status}`);
    return await res.json();
  } catch (err) {
    return { error: err.message };
  }
