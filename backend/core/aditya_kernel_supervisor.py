...
"""
Process & Thread Supervisor watchdog for Aditya.
...
logger = logging.getLogger("aditya-supervisor")
logger.setLevel(logging.INFO)

class AdityaKernelSupervisor:
    def __init__(self):
    ...
    def start(self):
        ...
        if self._running:
            return
        self._running = True
        self._supervisor_thread = threading.Thread(target=self._run_loop, name="AdityaSupervisor", daemon=True)
        self._supervisor_thread.start()
        logger.info("Aditya Supervisor watchdog started.")
...
supervisor = AdityaKernelSupervisor()
