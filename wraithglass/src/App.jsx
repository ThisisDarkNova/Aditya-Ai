...
import Sidebar from './SpringBoard/Organisms/Sidebar';
import StatusBar from './SpringBoard/Molecules/StatusBar';
import Visualizer from './SpringBoard/Organisms/Visualizer';
import BottomControls from './SpringBoard/Molecules/BottomControls';
import BootSequence from './SpringBoard/Molecules/BootSequence';
import { useBackend } from './CoreMotion/useBackend';
import { AnimatePresence, motion } from 'framer-motion';

// Lazy load pages
const Home     = lazy(() => import('./BespokeViews/Home/index'));
...
function ConnectionToast({ isConnected, consecutiveErrors }) {
  ...
  useEffect(() => {
    ...
    if (!isConnected && wasConnected) {
      // Just disconnected
      setMsg('VESPERA lost connection to Gemini');
      setType('error');
      setVisible(true);
      if (hideTimeout.current) clearTimeout(hideTimeout.current);
    } else if (isConnected && !wasConnected) {
      // Just reconnected
      setMsg('VESPERA reconnected ✓');
      setType('ok');
      setVisible(true);
...
import AmbientCanvas from './SpringBoard/Organisms/AmbientCanvas';
      ...
      <AnimatePresence>
        {showStartup && (
          <motion.div key="startup" exit={{ opacity: 0 }} transition={{ duration: 0.8 }} style={{ position: 'absolute', inset: 0, zIndex: 99999 }}>
            <BootSequence onComplete={handleStartupComplete} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {!showStartup && (
        <div className="flex w-full h-full relative" style={{ paddingTop: 32 }}>
          ...
          <div className="absolute left-4 top-10 bottom-4 z-50">
            <Sidebar setActivePage={setActivePage} activePage={activePage} status={backend.status} isConnected={backend.isConnected} aiState={resolvedAiState} />
          </div>
                ...
                <AnimatePresence mode="wait">
                  {activePage && activePage !== 'Visualizer' && (
                    ...
                    >
                      ...
                      {activePage === 'Profile' && <Profile memory={backend.memory} saveWraithglassMemoryEngine={backend.saveWraithglassMemoryEngine} chatHistory={backend.chatHistory} realtimeStats={backend.realtimeStats} />}
                      ...
                      {activePage === 'SkillTree' && <SkillTree isConnected={backend.isConnected} />}
                    </motion.div>
                  )}
                </AnimatePresence>
              </Suspense>
