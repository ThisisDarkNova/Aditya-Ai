...
import SystemSkillsWidget from './SystemSkillsWidget';
...
export default function Home({
  ...
  const handleSkillTrigger = (skillId) => {
    // Send a silent background chat command to the backend to trigger the skill
    onChat(`[SYSTEM_SKILL_TRIGGER] ${skillId}`);
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ padding: '80px 120px 240px' }}>
      <motion.div variants={stagger} initial="hidden" animate="visible"
        ...
        {/* ── VESPERA System Skills Widget ── */}
        <SystemSkillsWidget onSkillTrigger={handleSkillTrigger} />
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: 8 }}>
          <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.08em', fontWeight: 500 }}>
            Press <kbd style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 5, padding: '1px 6px', fontSize: 10, fontFamily: 'monospace' }}>Ctrl+Alt+J</kbd> to wake VESPERA
          </span>
        </div>
        <form onSubmit={handleCommand} className="glass-panel"
          ...
          <input type="text"
            placeholder={isListening ? 'Listening...' : 'Ask Wraithglass anything...'}
