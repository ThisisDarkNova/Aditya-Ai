...
import {
  fetchStatus, fetchRealtime, fetchWraithglassMemoryEngine,
  sendChat, sendClose, fetchSettings,
  saveSettings as apiSaveSettings,
  updateWraithglassMemoryEngine as apiUpdateWraithglassMemoryEngine,
  clearChatHistory,
...
export function useBackend() {
  ...
  const [memory, setWraithglassMemoryEngine] = useState({
    profile: {},
    history: [],
    goals: [],
    preferences: {},
  ...
  useEffect(() => {
    ...
    const memoryInterval = setInterval(async () => {
      const data = await fetchWraithglassMemoryEngine();
      if (!mountedRef.current) return;
      if (!data._error) setWraithglassMemoryEngine(data);
    }, 5000);

    // Initial batch fetch
    (async () => {
      const [s, r, m, setts] = await Promise.all([
        fetchStatus(), fetchRealtime(), fetchWraithglassMemoryEngine(), fetchSettings()
      ]);
      if (!mountedRef.current) return;
      ...
      if (!m._error) setWraithglassMemoryEngine(m);
  ...
  const chat = useCallback(async (text) => {
    ...
    if (!ok) {
      setSendError('Failed to reach Wraithglass. Is the backend running?');
  ...
  const saveWraithglassMemoryEngine = useCallback(async (updated) => {
    const res = await apiUpdateWraithglassMemoryEngine(updated);
    if (!res.error) {
      const m = await fetchWraithglassMemoryEngine();
      if (mountedRef.current && !m._error) setWraithglassMemoryEngine(m);
      return true;
    }
    return false;
  }, []);
