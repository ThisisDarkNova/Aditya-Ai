...
export default function Browser() {
  const [url, setUrl] = useState('https://github.com/amdarknova-dev/Aditya-Ai');
  const [inputUrl, setInputUrl] = useState('https://github.com/amdarknova-dev/Aditya-Ai');
  const [history, setHistory] = useState(['https://github.com/amdarknova-dev/Aditya-Ai']);
  ...
  const handleRefresh = () => {
    const frame = document.getElementById('aditya-browser-frame');
    if (frame) {
      frame.src = url;
    }
  };
  ...
  const quickLinks = [
    { name: 'GitHub Repo', url: 'https://github.com/amdarknova-dev/Aditya-Ai' },
    { name: 'Aditya Docs', url: 'http://localhost:7865/docs' },
    ...
    <div className="w-full h-full flex flex-col p-6 text-white bg-transparent font-sans" style={{ backdropFilter: 'blur(30px)' }}>
      ...
      <div className="flex flex-col gap-4 p-4 mb-4 rounded-2xl border border-white/10 bg-white/5 shadow-2xl">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-serif tracking-wide text-slate-200">Aditya Portal Viewer</h2>
        </div>
